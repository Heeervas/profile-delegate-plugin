...
def test_tool_allowlist_bounds_and_default_privacy(tmp_path):
    ...
    for forbidden in ("SUPER_SECRET", "/private/path", "arguments", "reasoning", '"result"', '"prompt"'):
...
def test_sanitize_text_removes_controls_and_invalid_unicode():
...
def test_reopen_partial_tail_and_corrupt_complete_line(tmp_path):
    journal = EventJournal(tmp_path, task_id="pd_test", ui_session_id="ui-1")
    journal.ingest(frame("tool.start", {"tool_id": "t1", "name": "terminal"})); journal.close()
    path = tmp_path / "events.jsonl"; path.write_bytes(path.read_bytes() + b'{"partial":')
    reopened = EventJournal(tmp_path, task_id="pd_test", ui_session_id="ui-1")
    reopened.ingest(frame("tool.complete", {"tool_id": "t1", "name": "terminal"})); reopened.close()
    assert [item["seq"] for item in records(path)] == [1, 2]
    path.write_bytes(path.read_bytes() + b"not-json\n")
    degraded = EventJournal(tmp_path, task_id="pd_test", ui_session_id="ui-1"); before = path.read_bytes()
...
def test_caps_marker_terminal_reserve_and_oversized_reduction(tmp_path):
    ...
    for index in range(10): journal.ingest(frame("tool.start", {"tool_id": str(index), "name": "x" * 500}))
    ...
    oversized.ingest(frame("tool.start", {"tool_id": "t", "name": "x" * 128})); oversized.close()
...
def test_symlink_and_write_failure_degrade_without_raise(tmp_path, monkeypatch):
    outside = tmp_path / "outside"; outside.write_text("safe", encoding="utf-8")
    linked = tmp_path / "linked"; linked.mkdir(); (linked / "events.jsonl").symlink_to(outside)
...
def test_pre_session_buffer_bounded_matching_only(tmp_path):
    ...
    journal.set_session("ui-1"); journal.close()
...
def test_terminal_flush_fsync_and_private_permissions(tmp_path, monkeypatch):
    calls = []; monkeypatch.setattr(os, "fsync", lambda fd: calls.append(fd))
    journal = EventJournal(tmp_path, task_id="pd_test", ui_session_id="ui-1", persist_message_text=True)
    journal.ingest(frame("message.delta", {"message_id": "m", "text": "pending"})); journal.finalize("failed", error_code="code")
